...
export const reportsService = {
  ...
  async hoursReport(params: { from?: string; to?: string; userId?: string }, userRole: string, requesterId: string) {
    ...
    const isElevated = userRole === 'admin' || userRole === 'ceo' || userRole === 'hr';
    const roleFilterU = isElevated ? "" : "AND EXISTS (SELECT 1 FROM project_members pm1 JOIN project_members pm2 ON pm1.project_id = pm2.project_id WHERE pm1.user_id = u.id AND pm2.user_id = ?)";
    const roleFilterWL = isElevated ? "" : "AND EXISTS (SELECT 1 FROM project_members pm WHERE pm.project_id = wl.project_id AND pm.user_id = ?)";
    ...
    const monthlyRes = await query(monthlySql, params.userId ? [from, to, params.userId, ...reqParam] : [from, to, ...reqParam]);
    ...
    const overtimeRes = await query(overtimeSql, [from, to, from, to, ...reqParam]);
    ...
    const projRes = await query(projSql, userRole === 'manager' ? [requesterId] : []);

    return {
      monthlyHours: (monthlyRes.rows as Record<string, unknown>[]).map(r => ({ month: r['month'], hours: Number(r['hours']) })),
      ...
      overtimeReport: (overtimeRes.rows as Record<string, unknown>[]).map(r => ({
        name: r['name'], initials: r['initials'], color: r['color'],
      ...
      employeeReport: (empRes.rows as Record<string, unknown>[]).map(r => ({
        ...
        totalHours: Math.round(Number(r['total_hours']) * 100) / 100, projects: Number(r['projects']),
        utilization: Math.min(100, Math.round(Number(r['total_hours']) / (40 * 4) * 100)),
      })),
      projectReport: (projRes.rows as Record<string, unknown>[]).map(r => ({
        id: r['id'], name: r['name'], status: r['status'],
        budgeted: Number(r['estimated_hours']), logged: Number(r['logged_hours']),
